...
export function DropdownMenu(
    ...
    const mouseUpOnSummoningButtonHandler = (event: MouseEvent) => {
        const target = event.target as Node;
        if (summoningButtonRef.current?.contains(target)) {
            if (menuRender) {
                setMenuRender(false);
    ...
    const mouseOverOnSummoningButtonHandler = (event: MouseEvent) => {
        const target = event.target as Node;
        if (summoningButtonRef.current?.contains(target)) {
            setMenuRender(true);
        }
    ...
    const mouseOutOnSummoningButtonHandler = (event: MouseEvent) => {
        const target = event.target as Node;
